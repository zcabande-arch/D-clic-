...
import { api, connect, ensureSession, getToken, setToken } from "./db.js";
  ...
  function renderGroup(){
    ...
    panel.append(el("p",{text:"Code à partager :"}),el("div",{class:"code",text:g.id}),
      el("p",{text:"Partage le lien de Déclic, puis ce code : tes proches créent leur profil et rejoignent le groupe."}),ul,
      el("div",{class:"row"},el("button",{class:"btn ghost small",onclick:()=>copy(g.id)},"Copier le code"),el("button",{class:"btn ghost small",style:"color:var(--late)",onclick:()=>leave(g)},"Quitter le groupe")));
  ...
  $("#cGo").addEventListener("click",async()=>{
    ...
    try{
      let code=genCode();for(let i=0;i<3;i++){if(!(await S.db.doc("groups/"+code).get()).exists)break;code=genCode();}
      await S.db.doc("groups/"+code).set({name:n,members:[S.uid],createdBy:S.uid,createdAt:Date.now()});
  ...
  $("#jGo").addEventListener("click",async()=>{
    ...
    try{
      const s=await S.db.doc("groups/"+code).get();
      if(!s.exists){$("#jErr").textContent="Aucun groupe avec ce code.";b.disabled=false;return;}
      const g=s.data();if(!g.members.includes(S.uid))await S.db.doc("groups/"+code).update({members:[...g.members,S.uid]});
  ...
  async function leave(g){
    ...
    try{await S.db.doc("groups/"+g.id).update({members:g.members.filter(m=>m!==S.uid)});closeGroup();toast("Groupe quitté");}catch(e){toast("Impossible de quitter le groupe.");}
  }

  // ---------- rappels et installation ----------
  ...
  async function enablePush(){
    try{
      if(await Notification.requestPermission()!=="granted"){toast("Notifications refusées dans ton navigateur.");return;}
      const reg=await navigator.serviceWorker.ready,{key}=await api("/api/push/key");
      ...
      await api("/api/push/subscribe",{sub:sub.toJSON(),tz:tz()});
      toast("Rappels activés : à chaque heure pile, de 8h à 20h");
    }catch(e){toast("Impossible d’activer les rappels.");}
  ...
  async function disablePush(){
    ...
    }catch(e){toast("Impossible de désactiver les rappels.");}
    S.push=await pushState();render();
  }
  ...
  function pushCard(){
    ...
    return el("div",{class:"empty invite"},
      el("p",{text:"Active les rappels pour être prévenu à chaque déclic, de 8h à 20h."}),
      el("div",{class:"row"},el("button",{class:"btn flash small",onclick:()=>{markPushAsked();enablePush();}},"Activer les rappels"),
        el("button",{class:"btn ghost small",onclick:()=>{markPushAsked();render();}},"Plus tard")));
  }
  function installCard(){
    if(!S.installEvt)return null;
    return el("div",{class:"empty invite"},el("p",{text:"Installe Déclic sur ton écran d’accueil pour l’ouvrir comme une vraie app."}),
  ...
  function settingsCard(){
    const rows=el("section",{class:"profile settings"},el("h2",{text:"Réglages"}));
    const st=S.push;
    let pushCtl;
    if(st==="on")pushCtl=el("button",{class:"btn ghost small",onclick:disablePush},"Désactiver");
    else if(st==="off")pushCtl=el("button",{class:"btn flash small",onclick:enablePush},"Activer");
    rows.append(el("div",{class:"setrow"},el("div",{},el("b",{text:"Rappels à chaque déclic"}),
      el("p",{text:st==="on"?"Activés sur cet appareil.":st==="denied"?"Bloqués : autorise les notifications de Déclic dans les réglages du navigateur.":st==="unsupported"?(isIOS?"Sur iPhone, ajoute d’abord Déclic à l’écran d’accueil (Partager › Sur l’écran d’accueil).":"Ce navigateur ne gère pas les notifications."):"Une notification à 8h, 9h… jusqu’à 20h."})),pushCtl||null));
    rows.append(el("div",{class:"setrow"},el("div",{},el("b",{text:"Utiliser Déclic sur un autre appareil"}),
      el("p",{text:"Copie ton lien personnel et ouvre-le sur l’autre appareil. Ne le partage avec personne : il donne accès à ton compte."})),
      el("button",{class:"btn ghost small",onclick:async()=>{const link=location.origin+"/#cle="+encodeURIComponent(getToken()||"");try{await navigator.clipboard.writeText(link);toast("Lien copié");}catch(e){prompt("Ton lien personnel :",link);}}},"Copier le lien")));
    return rows;
  }
  ...
  // ---------- boot ----------
  (async()=>{
    const hash=new URLSearchParams(location.hash.slice(1)),key=hash.get("cle");
    if(key){
      history.replaceState(null,"",location.pathname);
      if(key!==getToken()&&(!getToken()||confirm("Ouvrir ton compte Déclic sur cet appareil ? Le compte actuel de cet appareil sera remplacé.")))setToken(key);
    }
    let uid;
    try{uid=await ensureSession();}
    catch(e){app.replaceChildren(el("p",{class:"notice",text:"Impossible de joindre le serveur Déclic. Vérifie ta connexion internet."}),el("div",{class:"row",style:"margin-top:12px"},el("button",{class:"btn",onclick:()=>location.reload()},"Réessayer")));return;}
    const db=connect(online=>{if(!online)toast("Connexion perdue, nouvelle tentative…");});
    S.db=db;S.uid=uid;S.prefillName="";
    db.collection("profiles").limit(1000).onSnapshot(s=>{const m={};s.docs.forEach(d=>m[d.id]=d.data());S.profiles=m;S.profilesLoaded=true;render();},()=>{S.profilesLoaded=true;render();});
    db.collection("groups").where("members","array-contains",uid).onSnapshot(s=>{S.groups=s.docs.map(d=>({id:d.id,...d.data()}));render();},e=>{if(e.code!=="unavailable")toast("Impossible de charger tes groupes.");});
    if("serviceWorker"in navigator){try{await navigator.serviceWorker.register("/sw.js");}catch(e){}}
    S.push=await pushState();
    if(S.push==="on"){try{const reg=await swReg(),sub=await reg.pushManager.getSubscription();await api("/api/push/subscribe",{sub:sub.toJSON(),tz:tz()});}catch(e){}}
    render();
  })();
